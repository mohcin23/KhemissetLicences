...
exports.approveUser = async (req, res) => {
  try {
    // Vérifier que l'utilisateur existe et n'est pas déjà actif
    const [rows] = await db.execute(
...
// PHASE 4 FINAL: Create employee (agent) account from admin panel
exports.createEmployee = async (req, res) => {
  try {
    const { full_name, username, password, role = 'agent' } = req.body;
    const allowedRoles = ['agent', 'lecteur'];
    ...
    const [result] = await db.execute(
      `INSERT INTO users (full_name, username, password_hash, role, is_active, approved_by, approved_at)
       VALUES (?, ?, ?, ?, 1, ?, NOW())`,
      [full_name.trim(), username.trim(), hashed, role, req.user.id]
    );
    ...
    await logAudit(req, {
      ...
      details: { full_name: full_name.trim(), username: username.trim(), role }
    });

    const [created] = await db.execute(
      'SELECT id, full_name, username, role, is_active, approved_by, approved_at, created_at FROM users WHERE id = ?',
      [result.insertId]
    );
...
// PHASE 4 FINAL: Get activity history for a user
...
exports.changeRole = async (req, res) => {
  try {
    const { role } = req.body;
    const allowed = ['admin', 'agent', 'lecteur', 'citizen'];
