...
const canGenerateForDemande = (req, demande) => {
  // Tout agent peut générer le PDF pour n'importe quelle demande
...
const getLogoHtml = () => {
  // Cherche le logo à plusieurs endroits possibles
...
// [MULTI-LICENCE] Convertit le statut DB en texte de décision pour les templates
...
// [MULTI-LICENCE] Génère les conditions selon le type et le statut
...
const buildTemplateData = (demande) => {
  const licenceType = demande.licence_type || 'pharmacie';

  // [MULTI-LICENCE] Lecture de extra_data en toute sécurité
  let extraData = {};
  try {
  ...
  // Numéro de décision : on génère un numéro formaté si absent
  ...
  const commonData = {
    ...
    // Signature (peut être surchargé par extra_data si renseigné)
  ...
  return {
    ...commonData,
    // Tous les champs extra_data (superficie, capacité, nom_etablissement, etc.)
    ...extraData
  };
};
...
const sendDecisionPdf = async (req, res, disposition = 'attachment') => {
  try {
    ...
    const filled = fillTemplate(template, buildTemplateData(d));
    const pdfBuffer = await generatePdf(filled);
    const filename = `decision_${d.numero_dossier}_${lang}.pdf`;

    // Log document generation (ignore error if table structure differs)
    try {
      await db.execute(
...
// /pdf/both/:id — returns only decision as base64 (decharge removed per request)
exports.generateBoth = async (req, res) => {
  try {
    ...
    const decisionPdf = await generatePdf(fillTemplate(decisionTemplate, buildTemplateData(d)));
...
exports.generateRapport = async (req, res) => {
  try {
    ...
    const [demandes] = await db.execute(`
      SELECT numero_dossier, nom_complet, cin, commune, cercle, statut, date_creation
      FROM demandes
    ...
    const demandesGrouped = Object.entries(grouped).map(([commune, items]) => `
      ...
      <table>
        <thead><tr><th>N dossier</th><th>Nom</th><th>CIN</th><th>Cercle</th><th>Statut</th><th>Date creation</th></tr></thead>
        <tbody>
          ${items.map(item => `
            <tr>
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="rapport_licences_pharmacies.pdf"');
    res.send(pdfBuffer);
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
exports.generateRapportMensuel = async (req, res) => {
  try {
    ...
    const [demandes] = await db.execute(
      `SELECT numero_dossier, nom_complet, cin, commune, statut
       FROM demandes
       WHERE date_creation >= ? AND date_creation < ?
    ...
    `).join('') : `
      <tr><td colspan="5" class="empty">Aucune demande pour ce mois</td></tr>
    `;
      ...
      <body>
        <div class="official-header">
          ...
          <div class="institution">
            ...
            <div class="sub">Systeme de gestion des licences pharmaceutiques</div>
          </div>
        </div>

        <h1>Rapport Mensuel &mdash; Licences Pharmaceutiques &mdash; ${escapeHtml(label)}</h1>
        ...
        <table>
          <thead><tr><th>N dossier</th><th>Nom</th><th>CIN</th><th>Commune</th><th>Statut</th></tr></thead>
